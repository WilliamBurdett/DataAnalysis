...
$conQuery = mysql_query("select value from visual_backend.consolidate_list where table_id=" . $_REQUEST['table_id']);
...
while($conResult = mysql_fetch_array($conQuery)){
	$where .= " and " . $tableResult['fieldName'] . " NOT REGEXP '^" . $conResult['value'] . "'";
	array_push($valueList, $conResult['value']." - All");
}
